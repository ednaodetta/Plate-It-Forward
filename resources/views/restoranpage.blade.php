...
<body class="bg-DefaultWhite overflow-x-hidden">
    ...
    <x-navbar></x-navbar>
    <div class="relative bg-bgbang bg-no-repeat bg-cover bg-center h-[80vh] flex flex-col justify-center items-center text-center pt-[19rem]">
        ...
        <div class="relative z-20 w-full flex justify-center items-center mt-10 px-4 sm:px-8">
            <div class="flex flex-col bg-white rounded-[20px] p-4 shadow-lg w-[90%] sm:w-[80%] md:w-[60%] lg:w-[50%] xl:w-[40%]">
                ...
                <div class="relative flex items-center">
                    ...
                    <ul id="location-list" class="hidden absolute left-0 top-full mt-1 bg-white w-full rounded-lg shadow-md max-h-40 overflow-y-auto z-50 text-left"></ul>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="flex overflow-x-auto no-scrollbar gap-5 px-5">
            @foreach ($flashSaleProducts as $item)
                ...
                <div class="flex-shrink-0 w-80 bg-white rounded-xl overflow-hidden shadow-md transition-all duration-300 hover:scale-105 hover:bg-gray-100 relative">
                    ...
                    <img class="w-full h-48 object-cover" src="https://via.placeholder.com/300" alt="{{ $item->name }}" />
                    <div class="p-4">
                        ...
                        <div class="flex items-center text-gray-600">
                            <span class="text-sm font-medium">Rp {{ number_format($item->price_discount, 0, ',', '.') }}</span>
        ...
        @foreach ($recommendedRestaurants as $restaurant)
            ...
            <div class="w-full sm:w-80 md:w-96 bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg hover:scale-105 transition-transform duration-300 ease-in-out">
                <a href="{{ route('menupage', ['id' => $restaurant->id]) }}">
                    <img class="w-full h-48 object-cover" src="{{ asset('assets/Image/Mcd.jpg') }}" alt="{{ $restaurant->name }}">
                    <div class="p-4">
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if (Array.isArray(data) && data.length > 0) {
                                    let uniqueLocations = [...new Set(data)]; // Buat data unik

                                    uniqueLocations.forEach(function(location) {
                                        dropdown.append(`<li class="p-2 hover:bg-gray-200 cursor-pointer" data-city="${location}">${location}</li>`);
                                    });
                                } else {
                                    dropdown.append(`<li class="p-2 text-gray-500">No results found</li>`);
                                }
                            },
