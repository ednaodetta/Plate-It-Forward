<x-html>
    ...
    <body class="bg-DefaultWhite flex flex-col items-center">
        ...
        <div class="w-[90%]">
            ...
            <h1 class="text-2xl font-bold mb-4 mt-10">Detail Produk: {{ $product->name }}</h1>
            ...
            <div class="w-11/12 mx-auto flex justify-between items-center">
                ...
                <div class="flex">
                    <a href="{{ route('productexp.create', ['product_id' => $product->id]) }}"
                        class="text-DefaultGreen text-3xl">
            ...
            <div class="overflow-x-auto mt-4">
                <table class="min-w-full bg-white border border-gray-300">
                    ...
                    <tbody>
                        @forelse ($productExps as $index => $exp)
                            <tr class="text-center">
                                <td class="border px-4 py-2 relative">
                                    <input type="checkbox" class="hidden checkbox-item" name="delete_ids[]"
                                        value="{{ $exp->id }}" style="margin-right: 8px;">
                                    <div class="action-menu">
                                        <button onclick="toggleActionMenu({{ $exp->id }})">⋮</button>
                                        <div id="menu-{{ $exp->id }}"
                                            class="absolute top-8 left-0 bg-white border shadow-md rounded hidden w-28 z-50">

                                            <a href="{{ route('productexp.edit', $exp->id) }}"
                                                class="block px-4 py-2 hover:bg-gray-100">Update</a>
                                            <form action="{{ route('productexp.delete') }}" method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-gray-500 border px-4 py-2 text-center">Belum ada data
                                    expiry date</td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="flex justify-between">
                ...
                <a href="{{ route('products') }}"
                    class="mt-4 inline-block bg-DefaultGreen text-white px-4 py-2 rounded">Kembali</a>


                <button id="delete-selected-btn" class="hidden bg-red-500 text-white px-4 py-2 rounded"
                    onclick="submitDeleteForm()">Hapus yang Dichecklist</button>
            </div>
